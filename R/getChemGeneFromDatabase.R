getChemGeneFromDatabase = function(ChemData,ChemDataType='inchikey',QEDset=0.2,geneScore=400,geneSelectPv=0.05,targetDatabase=c('HIT','TCMID','STITCH','TCMSP','CUSTOM'),Lipinski_Viosset=0,Veber_Viosset=0){
  #ChemData:data.frame:colnames-->'cid','chemical_name','key'.'key' is smiles or inchikey
  #ChemDataType:'inchikey' or 'smiles'
  #result:list(combinedGeneID=selectGeneID,GeneID_split=tempR_split,subD3=subD3):$GeneID_split-->data.frame(cid,chemical_name,inchikey,geneID).$subD3-->data.frame(cid,geneID,score,database,QED_DES,Lipinski_Vios,Veber_Vios,chemical_name,inchikey)
  require(data.table)
  require(stringr)
  require(RSQLite)
  require(plyr)
  ...
  if (nrow(ChemData2)>0){
    ...
    subD0 <- fetch(res, n =-1)
    dbDisconnect(tmp)
    subD=join(subD0[,c('geneID','score','inchikey','database','QED_DES',"Lipinski_Vios","Veber_Vios")],ChemData2,by='inchikey')
    ...
    subD2=unique(subD[,.(id=cid,geneID=geneID,score,database,QED_DES=QED_DES[1],Lipinski_Vios=Lipinski_Vios[1],Veber_Vios=Veber_Vios[1]),by=c('cid','geneID')])
    tempR=geneScoreCal(subD2,Pcutoff = geneSelectPv)
    tempR_split=subD2[,.(geneID=toString(geneID),QED_DES=QED_DES[1],Lipinski_Vios=Lipinski_Vios[1],Veber_Vios=Veber_Vios[1]),by='id']
    tempR_split=plyr::rename(tempR_split,c('id'='cid'))
    tempR_split=join(tempR_split,ChemData2,by='cid')
    tempR_split=tempR_split[,.(cid,chemical_name,inchikey,geneID,QED_DES,Lipinski_Vios,Veber_Vios)]
    selectGeneID=tempR$selectGeneId###selectGeneID
    subD3=subD2[,.(cid,geneID,score,database,QED_DES,Lipinski_Vios,Veber_Vios)]
